...
void draw_menu_line(std::string caption, float lineWidth, float lineHeight, float lineTop, float lineLeft, float textLeft, bool active, bool title, bool rescaleText = true)
{
	...
	//Change colours or events when hovered over a certain option.
	if (active)
	{
...
{
	if (a) *a = get_key_pressed(VK_SPACE) || get_key_pressed(VK_NUMPAD5);
	if (b) *b = get_key_pressed(VK_BACK) || trainer_switch_pressed() || get_key_pressed(VK_NUMPAD0);
	if (up) *up = get_key_pressed(VK_UP) || get_key_pressed(VK_NUMPAD8);
	if (down) *down = get_key_pressed(VK_DOWN) || get_key_pressed(VK_NUMPAD2);
	if (r) *r = get_key_pressed(VK_RIGHT) || get_key_pressed(VK_NUMPAD6);
	if (l) *l = get_key_pressed(VK_LEFT) || get_key_pressed(VK_NUMPAD4);
}
